package paige.navic.data.database

import paige.navic.data.database.dao.*

object DbContainer {
	private lateinit var database: CacheDatabase

	fun setup(db: CacheDatabase) {
		database = db
	}

	val albumDao: AlbumDao by lazy { database.albumDao() }
	val genreDao: GenreDao by lazy { database.genreDao() }
	val playlistDao: PlaylistDao by lazy { database.playlistDao() }
	val songDao: SongDao by lazy { database.songDao() }
	val artistDao: ArtistDao by lazy { database.artistDao() }
	val lyricDao: LyricDao by lazy { database.lyricDao() }
	val syncActionDao: SyncActionDao by lazy { database.syncActionDao() }
}